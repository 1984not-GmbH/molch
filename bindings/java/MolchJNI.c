...
static void print_info_error(char *callFunct, return_status retStatus) {
	...
	char* error_message = molch_print_status(&error_message_length, retStatus);
	if ((error_message == NULL) || (error_message_length > INT_MAX)) {
		android_only(__android_log_print(ANDROID_LOG_DEBUG, callFunct, "error;Failed to print error message.\n");)
		return;
	}
	android_only(__android_log_print(ANDROID_LOG_DEBUG, callFunct, "error;%.*s\n", (int)error_message_length, error_message);)
}
