...
int decrypt_message(
	...
	//first byte of the packet is the header length
	...
	if ((PRE_CIPHERTEXT_LENGTH + crypto_secretbox_MACBYTES + 255) > packet_length) {
		//packet isn't long enough (header field has incorrect value [probably])
		return -10;
	}
	...
	status = crypto_onetimeauth_verify(
			packet + HEADER_LENGTH + 1 + crypto_secretbox_NONCEBYTES, //MAC
			packet, //input
