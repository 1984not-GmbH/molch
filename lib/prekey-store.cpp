...
static void node_init(PrekeyStoreNode * const node) {
	if (node == nullptr) {
		return;
	}

	node->private_key.init(node->private_key_storage, PRIVATE_KEY_SIZE, 0);
	node->public_key.init(node->public_key_storage, PUBLIC_KEY_SIZE, 0);
	node->next = nullptr;
 ...
 */
return_status PrekeyStore_create(PrekeyStore ** const store) {
	return_status status = return_status_init();

	if (store == nullptr) {
		THROW(INVALID_INPUT, "Invalid input to PrekeyStore_create.");
	}
...
static void node_add(PrekeyStore * const store, PrekeyStoreNode * const node) {
 ...
 */
static int deprecate(PrekeyStore * const store, size_t index) {
	int status = 0;
	//create a new node
...
return_status PrekeyStore_get_prekey(
	...
	if (found_prekey == nullptr) {
		deprecated = true;
		PrekeyStoreNode *next = store->deprecated_prekeys;
		while (next != nullptr) {
 ...
 */
return_status PrekeyStore_rotate(PrekeyStore * const store) {
	return_status status = return_status_init();

	int64_t current_time = time(nullptr);

	if (store == nullptr) {
		THROW(INVALID_INPUT, "Invalid input to PrekeyStore_rotate: store is nullptr.");
	}
	...
	//At least one key to be removed
	{
		...
		if ((store->deprecated_prekeys != nullptr) && (store->oldest_deprecated_expiration_date < current_time)) {
			PrekeyStoreNode **last_pointer = &(store->deprecated_prekeys);
			PrekeyStoreNode *next = store->deprecated_prekeys;
			while(next != nullptr) {
...
void PrekeyStore_destroy(PrekeyStore * const store) {
	...
	while (store->deprecated_prekeys != nullptr) {
		PrekeyStoreNode *node = store->deprecated_prekeys;
 ...
 */
static size_t count_deprecated_prekeys(PrekeyStore * const store) {
	size_t length = 0;
	for (PrekeyStoreNode *node = store->deprecated_prekeys; node != nullptr; node = node->next, length++) {}

	return length;
}

return_status PrekeyStore_export_key(PrekeyStoreNode* node, Prekey ** const keypair) __attribute__((warn_unused_result));
return_status PrekeyStore_export_key(PrekeyStoreNode* node, Prekey ** const keypair) {
	return_status status = return_status_init();
	...
	if ((node == nullptr) || (keypair == nullptr)) {
		THROW(INVALID_INPUT, "Invalid input to PrekeyStore_export_key.");
	}
...
return_status PrekeyStore_export(
	...
	if ((store == nullptr) || (keypairs == nullptr) || (deprecated_keypairs == nullptr)) {
		THROW(INVALID_INPUT, "Invalid input to PrekeyStore_export.");
	}
	...
	*keypairs = (Prekey**)zeroed_malloc(PREKEY_AMOUNT * sizeof(Prekey*));
	...
	//normal keys
	for (size_t i = 0; i < PREKEY_AMOUNT; i++) {
		status = PrekeyStore_export_key(&(store->prekeys[i]), &(*keypairs)[i]);
	...
	//deprecated keys
	{
		...
		for (size_t i = 0; (i < deprecated_prekey_count) && (node != nullptr); i++, node = node->next) {
			status = PrekeyStore_export_key(node, &(*deprecated_keypairs)[i]);
...
static return_status PrekeyStoreNode_import(PrekeyStoreNode * const node, const Prekey * const keypair) {
	return_status status = return_status_init();

	//check input
	if ((node == nullptr) || (keypair == nullptr)) {
		THROW(INVALID_INPUT, "Invalid input to PrekeyStoreNode_import.");
	}
...
return_status PrekeyStore_import(
	...
	*store = (PrekeyStore*)sodium_malloc(sizeof(PrekeyStore));
	...
	for (size_t i = 0; i < keypairs_length; i++) {
		status = PrekeyStoreNode_import(&((*store)->prekeys[i]), keypairs[i]);
	...
	for (size_t i = 1; i <= deprecated_keypairs_length; i++) {
		...
		status = PrekeyStoreNode_import(deprecated_keypair, deprecated_keypairs[deprecated_keypairs_length - i]);
