...
int main() {
	try {
		...
		auto buffer1{pool.allocate<std::byte>(ProtobufPoolBlock::default_block_size - 10)};
		std::cout << "buffer1 = " << reinterpret_cast<void*>(buffer1) << std::endl;
		auto buffer2{pool.allocate<std::byte>(10)};
		...
		auto in_new_block{pool.allocate<std::byte>(1)};
		...
		auto large{pool.allocate<std::byte>(2 * ProtobufPoolBlock::default_block_size)};
		std::cout << "large = " << reinterpret_cast<void*>(large) << std::endl;
		auto fill_gap{pool.allocate<std::byte>(2)};
