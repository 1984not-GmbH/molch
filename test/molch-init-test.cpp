...
int main(int argc, char *args[]) noexcept {
	try {
		...
		gsl::span<gsl::byte> backup_buffer{uchar_to_byte(backup.get()), narrow(backup_length)};
		gsl::span<gsl::byte> backup_key_buffer{uchar_to_byte(backup_key), BACKUP_KEY_SIZE};
